...
- **`daily.executor` `gpt-5.6-terra:high` → `gpt-5.6-luna:max`**. 사용자 정책 결정이며
  **품질 비교는 FAIL 이었다.** 사전등록 60콜에서 효과크기 0/10, 토큰 47.1 vs 32.2,
  30쌍 중 29쌍이 정확히 동점이라 검정력이 사실상 없다. Wilcoxon 은 동점 때문에
  정규근사로 후퇴하므로 `p=0.1587` 은 근사값이고, 유효쌍 1개의 정확검정 단측 p 는
  0.5 다. 측정된 승리가 아니라 **정책**으로 채택했다 —
...
### Tooling (게이트 자체의 결함 — cyber-cop 패널 5회차까지 반영)

게이트가 "검사한다" 고 말하고 실제로는 안 하던 구간을 닫았다. 전부 음성 경로로 실증했다.
...
- 1회용 마이그레이션 `apply-roster-v3.py`·`apply-validator-v3.py` 삭제(적용 완료·self-refuse).
  `/tmp/_v3gate_*.txt` 예측 가능 이름 → `mktemp -d` + EXIT trap.
