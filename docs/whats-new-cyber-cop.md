...
comment-only 리뷰 자동화에 필요한 GitHub Actions 권한은 이게 전부다:

```yaml
permissions:
  contents: read
  pull-requests: write   # 리뷰 코멘트 게시용 — 읽기만 하면 read
```

`contents: write`, admin, 광범위 classic `repo` 토큰을 리뷰 봇에 주지 마라. 로컬 `gh`는 대상 레포 한정 fine-grained 토큰 권장.
...
| 카탈로그·설계 근거 | [README §5](../README.md#5-%EF%B8%8F-최종-카탈로그-13종) |
