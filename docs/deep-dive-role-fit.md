...
- **executor 축은 v1.4에서 교체됐다.** **Fable 5가 SWE-bench Verified 95.0 / SWE-Bench Pro 80.0으로 신 1위**(Opus 4.8 88.6 · GPT-5.5 82.6; Pro 80.3은 별도 열의 Mythos 5 — [errata E1](../evidence/2026-07-10-errata.md)). 당시 판단은 Opus 4.8 이 "구독 포함 코딩 최강" 이었다(**v2.1.0 에서 좌석은 Opus 5 로 승격**). Fable 5 는 **2026-07-20** 부터 Max/premium Team 이 주간 한도의 50% 를 포함하고 Pro/standard 는 usage credits 이라, 상시 executor 로는 비용 구조가 다르다(v2 에선 `dream-team`/`escalation` 채용 — **v3.0.0 에서 `dream-team` 이 삭제돼 `escalation` 만 남았다**). 터미널·에이전틱 축은 이제 **GPT-5.6 Sol이 SOTA**(Terminal-Bench 2.1 88.8) — 단 ⚠METR이 Sol의 SWE 평가 게이밍(역대 최고 비율)을 적발해 SWE류 점수는 할인해서 읽을 것. daily executor 는 동가 후계 `gpt-5.6-terra` 였다 — **v3.0.0 에서 `gpt-5.6-luna:max` 로 교체**(validator D-1 이 Luna 를 `:max` 단독으로 강제. like-for-like 우위는 미측정이라 정책 좌석이다 — [whats-new-v3](./whats-new-v3.md)).
- **xAI `grok-composer-2.5-fast`·`grok-code-fast-1` 은 저가·throughput 전용.** 독립 벤치 미공개/인플레이션으로 프론티어 코더가 아니며 grok-code-fast-1은 은퇴 예정 — executor 코어 비포함이 맞다.
- **default = Anthropic 플래그십.** 당시 Opus 4.8 이 Vals Index 종합(서빙 모델 중 1위)으로 재확인됐다 — **v2.1.0 출하 좌석은 Opus 5** 다. Fable 5 default(`dream-team`)는 품질 상한을 한 단계 더 올렸지만 refusal 분류기·credits 과금 캐비앗을 동반했다([§5](../README.md#프로필별-설계-근거)) — **v3.0.0 에서 `dream-team` 이 삭제돼 Fable 5 default 좌석은 없다.** `ultimate-sol` 의 Sol 라우터도 opt-in 실험이었으나(WARN 표면화) **v3.0.0 에서 삭제됐다 — 비-Anthropic 라우터는 `anthropic` 을 요구하지 않는 `budget`(Terra) 하나뿐이다.**
...
- **gap3 — GLM-5.2 vs DeepSeek:** 원래 판단은 eco executor=DeepSeek V4 Flash 유지(GLM-5.2 $1.40/$4.40 = 입력 10×·출력 15×, eco엔 과함)였다. **v2.1.0 에서 뒤집혔다** — 2026-08-16·17 실호출에서 `deepseek-v4-flash` 가 이 계정 **403 China opt-in** 이라 출하 좌석 자격을 잃었고, eco executor 도 `opencode-go/glm-5.2` 로 교체했다(코딩 프로브 8/8·5.1s, `evidence/2026-08-17-eco-executor.md`). **monorepo critic 도 `opencode-go/glm-5.2`** — 신규 오픈웨이트 1위(AA Index 51; DeepSeek V4 Pro 52→**44 하락**), cross-family 독립 유지, opencode-go 구독 내 서빙이라 한계비용 낮음. 지역 정책이 풀리면 저가 우선 관점에서 deepseek 재평가. **v3.0.0**: `eco` 가 삭제돼 그 GLM-5.2 executor 좌석은 신설 `budget.executor` 로 이관됐다(`budget.planner` 는 `opencode-go/qwen3.8-max`). `monorepo.critic` 의 GLM-5.2 는 그대로다.
