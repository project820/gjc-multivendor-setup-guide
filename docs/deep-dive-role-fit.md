...
- **default = Anthropic 플래그십.** 당시 Opus 4.8 이 Vals Index 종합(서빙 모델 중 1위)으로 재확인됐다 — **v2.1.0 출하 좌석은 Opus 5** 다. Fable 5 default(`dream-team`)는 품질 상한을 한 단계 더 올리지만 refusal 분류기·credits 과금 캐비앗을 동반한다([§5](../README.md#프로필별-설계-근거)). `ultimate-sol`의 Sol 라우터는 opt-in 실험(WARN 표면화).
