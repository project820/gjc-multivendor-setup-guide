...
이 한 줄이 **12개 프로필을 `~/.gjc/agent/models.yml` 에 안전 병합**하고 기본 프로필을 `daily` 로 설정한다. 기존 설정은 자동 백업되며, 재실행해도 깔끔히 갱신된다.
...
**GJC 0.7.10 실효 기준**이다 — API 공식 스펙과 다른 곳이 있다(아래 각주):
...
Grok 4.x (xai)          minimal low medium high              ← grok-4.3 :xhigh는 high로 침묵 클램프
...
> [!IMPORTANT]
> **하드룰 5선**: ① Gemini Pro는 `low`/`high`만 ② openai-codex ctx는 **모델별** — `gpt-5.4`=**1M** · `gpt-5.5`=**272K**(400K→272K 축소; 구 "codex 272k" 일괄 룰 폐기) ③ Sonnet(4.6/5)은 GJC에서 `xhigh`/`max` 불가 · **Fable 5는 `max` 불가**(각각 high/xhigh로 클램프) ④ opencode-go는 `:effort` 생략(deepseek-v4 계열만 예외적으로 지원) ⑤ xai `grok-4.3` 상한은 `high`(`:xhigh` 침묵 클램프 — xhigh는 grok-build 프로바이더 전용인데 그쪽은 effort 서픽스 자체가 해석되지 않는다). 범위 밖 등급은 에러가 아니라 **클램프**된다.
>
> **각주(상류 갭)**: Claude 5 패밀리는 API 공식으로는 둘 다 `max`까지 지원한다. GJC 0.7.10 파서가 fable 패밀리를 몰라 폴백 추론하고 sonnet-5가 4.6 클램프를 상속하는 **엔진 쪽 갭**이며, 재현 자료와 함께 **상류에 제보됨**. 이 가이드는 GJC 실효 기준으로 기재한다.
...
| gpt | `openai-codex` | **ChatGPT 계정 → base GPT(gpt-5.5/5.4) 제공**. ctx: gpt-5.4=1M · gpt-5.5=272K |
...
| default (툴콜·정직성) | **Opus 4.8 / Fable 5** | 라우터 품질 = 전체 상한 (Fable은 refusal·과금 캐비앗 — [§5](#5--최종-카탈로그-12종)) |
...
| 🔥 **ultimate-f5** | Fable 5 중심 이벤트판 — **구독 포함 ~2026-07-07** | 이벤트 기간 최고 정확성 |
| 👑 **legend** | 본체만 Fable 5, 나머지는 지속 구조 | 7/7 이후에도 유지 가능한 최강 |
...
| 🗺️ **monorepo** | 전역 ≥1M ctx (gpt-5.5 배제) | 거대 코드베이스 |
| 🧱 **solo-anthropic** | 전 역할 Opus (v1.4: critic도 Opus) | 단일 벤더로만 운영 |
| 🤖 **solo-openai** | 전 역할 base GPT (5.5=272K · 5.4=1M) | ChatGPT만 구독 |
...
profiles:
  ...
  daily:                               # ★ 평소 기본 (--default daily)
    ...
    model_mapping:
      ...
      executor:  openai-codex/gpt-5.4:high                      # 코딩특화·중가($2.5/15)·벤더분산(검증✅)
  ...
  ultimate-f5:                         # 🔥 이벤트: Fable 5 중심 — 구독 포함 ~2026-07-07(주간 한도 50% 상한), 이후 usage credits($10/$50)
    required_providers: [anthropic, openai-codex, google-antigravity, xai]
    model_mapping:
      ...
      planner:   openai-codex/gpt-5.5:xhigh                     # 추론 축은 Fable 우위 미입증(ARC-AGI-2 미공개) — GPT 유지
      architect: google-antigravity/gemini-3.1-pro-low:high     # 멀티모달 검증 우위 유지(Fable 비전은 vendor-claimed)
      critic:    xai/grok-4.3:high                              # cross-family 불변 — Fable 자기평가 금지
      # 7/7 이후: default를 opus-4-8:high로 내리면 legend와 동일한 지속 구조

  legend:                              # 👑 Ultimate Legend — 7/7 이후에도 유지 가능한 최강 (fable은 default 한 자리만)
  ...
  escalation:                          # 고실패비용 — 실패 신호 시 최강 실행자(Fable 5) 투입 + critic 멀티벤더 병렬패널(§9)
    ...
    model_mapping:
      ...
      planner:   openai-codex/gpt-5.5:xhigh
      architect: google-antigravity/gemini-3.1-pro-low:high
      critic:    xai/grok-4.3:high                              # + 3표 교차벤더 critic 패널(독립투표→본체집계). ⚠xai :xhigh는 high로 침묵 클램프라 미사용
  ...
  monorepo:                            # 거대 코드베이스 — 전역 1M ctx (★gpt-5.5 272K 배제 — gpt-5.4는 1M이나 Opus가 동급 이상)
  ...
  solo-openai:                         # ChatGPT(Codex) 계정만 — base GPT 전용
    ...
    model_mapping:
      default:   openai-codex/gpt-5.5:high                      # 라우터(base GPT 최강, ctx 272K)
      executor:  openai-codex/gpt-5.5:xhigh                     # 이 계정 최강 코더
      planner:   openai-codex/gpt-5.5:xhigh                     # 최상위 추론
      architect: openai-codex/gpt-5.4:high                      # ★gpt-5.4 = 1M ctx — 대용량 입력은 이쪽(5.5는 272K)
  ...
  claude-codex:                        # ★2벤더(Claude+Codex)만 — 평소 균형. Anthropic=실행·ctx / Codex=추론·비평
    ...
    model_mapping:
      ...
      architect: anthropic/claude-opus-4-8:high                 # 1M 윈도우(gpt-5.5 272K 한계 회피)
...
# ─────────────────────────────────────────────────────────────────────────────
# Claude 5 패밀리(2026-07-01~): Fable 5 = $10/$50(Opus 2배), 구독 포함은 ~7/7(주간 한도 50%)뿐 — "무료" 아님.
...
- **ultimate** — 비용을 포기하고 executor까지 Opus `:max`. planner는 OpenAI 다양성을 위해 `gpt-5.5:xhigh`. **지속 가능판** — Fable 5 이벤트판은 `ultimate-f5`, 지속판 최상위는 `legend`.
- **ultimate-f5** — 🔥 **Fable 5 이벤트판**. Fable 5는 **2026-07-07까지 구독에 포함**(Pro/Max/Team, **주간 사용 한도의 50% 상한** — "무료"가 아니다. 헤비 유저는 7/7 전에 상한을 소진할 수 있다), **이후엔 usage credits 별도 과금**($10/$50, Opus의 2배). executor는 GJC 실효 최고 등급 `:xhigh`(`:max`는 xhigh로 침묵 클램프). **refusal 유의**: 신형 안전 분류기가 HTTP 200 + `stop_reason: refusal`로 거부할 수 있어 에이전트 하네스가 "빈 응답"으로 오인할 위험이 있다. **7/7 이후 다운그레이드 경로**: `legend`로 전환하거나 default를 `opus-4-8:high`로 내리면 `ultimate`와 동일한 지속 구조.
...
- **solo-openai** — ChatGPT(Codex) 계정만 쓰는 사용자용. 전 역할을 base GPT(gpt-5.5/5.4)로 — `-codex` 변종은 이 계정에서 미지원이라 제외. ctx는 모델별: gpt-5.5=272K, **gpt-5.4=1M**(대용량 입력은 architect gpt-5.4로). critic이 동일 벤더라 독립성은 약함.
- **claude-codex** — Claude+Codex **2구독만** 보유한 사용자용 Best 믹스. **Anthropic = 실행·컨텍스트**(default·executor·architect: Opus가 구독 포함 코딩 최강 SWE-bench 88.6 + 1M 윈도우 — gpt-5.5는 272K라 대형 ctx는 Opus가 유리), **Codex = 추론·비평**(planner: GPT-5.5 추론 플래그십 / critic: GPT가 Opus executor를 cross-family 독립 비평). 2벤더로도 5역할 cross-family critic 성립.
...
> 모든 셀렉터를 이 환경에서 `gjc -p --no-session --no-tools --model <sel> "..."` 로 **실제 호출**해 동작을 확인했다(**최종 검증 2026-07-02, gjc 0.7.10** — 별도 표기 없는 항목은 2026-06-18 검증분, 핵심 셀렉터는 0.7.10 회귀 배터리로 재검증(07-02✅ 표기)). "동작함"의 근거는 추정이 아니라 실호출이다.
...
| `xai` | `grok-4.3:high`(07-02 재검증✅ — `:xhigh`는 high로 침묵 클램프) · `grok-4-1-fast:high`(07-02✅) · `grok-4-fast:high`(07-02✅) · `grok-code-fast-1` · `grok-composer-2.5-fast` |
| `grok-build` | `grok-4.3`(**bare 셀렉터만** — effort 서픽스 미해석, 07-02✅. SuperGrok OAuth) |
| `google-antigravity` | `gemini-3.1-pro-low`(±`:high`, 07-02 재검증✅) · `gemini-3.5-flash-low`(**신규 핀**, 07-02✅) · `gemini-3.5-flash`(퍼지 매칭 동작, 07-02✅) · `gemini-3-flash` · `claude-opus-4-6-thinking`(06-18 — Claude 5 출시 후 번들 구성 재확인 전) |
...
> [!WARNING]
> **이 환경에서 동작하지 않은 셀렉터**(피하라): `openai-codex/gpt-5.3-codex`·`gpt-5.2-codex`·`gpt-5.1-codex-max`·`gpt-5.1-codex-mini`(ChatGPT 계정 미지원) · `google-antigravity/gemini-3.1-pro-high`(**0.7.10 카탈로그 목록엔 나타나지만 호출은 여전히 400** — 문서화된 함정 유지, 엔진은 `gemini-3.1-pro-low:high` 사용) · `gemini-3-pro`(은퇴) · `claude-sonnet-4-6-thinking`(404) · `gpt-oss-120b`(500). `opencode-go/*` 는 `OPENCODE_API_KEY` **미설정 시에만** 실패한다(키 설정 후 위 표대로 동작). 참고: `fable-5:max`·`sonnet-5:xhigh/max`·`grok-4.3:xhigh` 는 실패가 아니라 **침묵 클램프**다([§3-2](#3-2-추론등급effort-치트시트)).
...
- **planner 추론 축은 분열한다.** GPQA Diamond(과학지식)는 상위권이 93~96%로 *saturated* — 단독 1위는 이제 **Sonnet 5(96.2)**, Gemini 3.1 Pro 94.3. ARC-AGI-2(추상·유동추론)는 **GPT-5.5가 명확히 앞선다**(0.850 vs 0.771; Fable 5는 미공개 — 추론 축 우위 미입증). 플래닝에 더 직결되는 유동추론(ARC) 기준 → ultimate/escalation `planner=GPT-5.5` 유지. **과학지식추론 비중이 크면 `planner → google-antigravity/gemini-3.1-pro-low:high`로 swap.** (후계자 임박: GPT-5.6 Sol 파트너 프리뷰 중 — GA 시 재검증.)
- **executor 축은 v1.4에서 교체됐다.** **Fable 5가 SWE-bench Verified 95.0 / SWE-bench Pro 80.3으로 신 1위**(Opus 4.8 88.6 · GPT-5.5 82.6). Opus 4.8은 "구독 포함 코딩 최강"으로 유지 — Fable 5는 7/7 이후 usage credits 과금이라 상시 executor로는 비용 구조가 다르다(`ultimate-f5`/`escalation`에서만 채용). DeepSWE/Terminal-Bench에선 GPT-5.5 강세 유지 — 터미널·에이전틱 위주면 GPT-5.5도 고려.
...
설치기가 하는 일: `~/.gjc/agent/models.yml` 에 12개 프로필 안전 병합(재실행 시 자동 갱신) · 기존 파일 자동 백업 · 기본 프로필 `daily` 설정. `curl` + `python3` 만 있으면 된다.
...
| 정확성 최우선 (Fable 5 이벤트 ~2026-07-07) | `ultimate-f5` → 이후 `legend` |
| 대량 리팩터·마이그레이션 | `eco` |
...
Gemini(`google-antigravity`)는 **Google AI Pro/Ultra 구독 토큰**으로 동작한다(per-token 청구가 아니라 구독에 포함). **Fable 5는 2026-07-07까지 Claude 구독(Pro/Max/Team)에 포함**되나 주간 사용 한도의 50% 상한이 걸리고, **이후엔 usage credits 별도 과금**이다 — "무료"가 아니다. 나머지는 per-token 과금이며, 주요 모델 단가는 다음과 같다($/1M, 입력/출력):
...
> † Fable 5는 Opus의 정확히 2배 단가. 구독 포함분(~7/7)도 주간 한도를 소모한다.
...
| 프로필 | 비용 | 핵심 비용 동인 |
|---|---|---|
| ultimate-f5 | ●●●●● | default·executor Fable 5 — ~7/7 구독 포함(주간 한도 50% 상한), 이후 credits $10/50 |
| legend | ●●●●● | default Fable 5(7/7 후 credits) + executor Opus `:max` |
| ultimate / escalation | ●●●●● | executor Opus `:max`/Fable `:xhigh`(구원투수) + planner GPT-5.5 `:xhigh` |
...
**Claude 5 패밀리** · [Fable 5 재배포 공지](https://www.anthropic.com/news/redeploying-fable-5) · [platform.claude.com 모델 문서](https://platform.claude.com/docs) — 가격·구독 포함(~7/7)·effort 스펙 교차 확인 2026-07-02
...
**v1.4** · [CHANGELOG](./CHANGELOG.md) · [유지보수·검증 플레이북](./MAINTAINING.md) · 라이선스 [CC BY 4.0](./LICENSE) · GJC = [Gajae Code](https://github.com/Yeachan-Heo/gajae-code)
