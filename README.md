...
이 한 줄이 **14개 프로필을 `~/.gjc/agent/models.yml` 에 안전 병합**하고 기본 프로필을 `daily` 로 설정한다. 기존 설정은 자동 백업되며, 재실행해도 깔끔히 갱신된다.
...
| default (툴콜·정직성) | **Opus 4.8 / Fable 5** | 라우터 품질 = 전체 상한 (Fable은 refusal·과금 캐비앗 — [§5](#5-️-최종-카탈로그-14종)). v1.12: `ultimate-sol`이 유일한 명시 예외(투트랙 B — Sol 라우터, 272K·툴콜 열세 트레이드오프) |
| critic (독립성) | **cross-family** | 메타-심판 > 토론형 집계 |

> † **후계 반영 각주**: planner 축 후계 GPT-5.6 Sol은 **GA(2026-07-09) 즉시 반영 완료** — v1.11에서 planner 세대교체, v1.12에서 투트랙 B(`ultimate-sol` 🌞) 신설(근거: [evidence/2026-07-10-two-track-sol-notes.md](./evidence/2026-07-10-two-track-sol-notes.md)). architect 축은 Gemini 3.5 Pro(2M ctx, Deep Think)가 7월 GA로 연기 — 출시 시 이 표는 재검증 대상이다.
...
| 🌞 **ultimate-sol** | **투트랙 B(실험적)** — Sol 본체·실행 + Fable 계획 + Opus 1M 레인 | 장기 자율 워크플로·터미널·사이버 중심, 272K로 충분할 때 |
...
profiles:
  ...
  ultimate-sol:                        # 🌞 투트랙 B (opt-in·실험적) — GPT-5.6 Sol 중심. default 비-Anthropic 첫 명시 예외 · L3 rolefit 사전 출하(OpenAI 자사표 기반 — 실측 후 조정)
    required_providers: [openai-codex, anthropic, xai]
    model_mapping:
      default:   openai-codex/gpt-5.6-sol:high                  # 장기 워크플로 완주 1위(Agents' Last Exam 52.7 vs Fable 40.5)·토큰효율. ⚠ctx 272K(1M 아님)·툴콜 축 열세(Toolathlon 58 vs Fable 61.7)
      executor:  openai-codex/gpt-5.6-sol:xhigh                 # 터미널·에이전틱 SOTA(T-B 2.1 88.8·DeepSWE 72.7·CTF 96.7). ⚠레포급(SWE-Pro 64.6 vs Fable 80.0)은 escalation으로
      planner:   anthropic/claude-fable-5:xhigh                 # 지능·심층추론 축 1위(AA-Int 59.9·GDPval 1759.6·FrontierMath T4 87.8) — cross-family 계획
      architect: anthropic/claude-opus-4-8:high                 # 1M ctx·MRCR 76% — 272K 본체의 장문맥 보완 레인(구독 포함)
      critic:    xai/grok-4.5:high                              # 3족 보존 — executor(gpt)·planner(claude) 모두와 cross-family
      # track 경계: 레포급 코딩·1M 세션·툴콜 신뢰성 = track A(ultimate/legend/escalation)
...
- **ultimate-sol** — 🌞 **투트랙 B** (v1.12 신규, opt-in·실험적). "Fable·Opus 중심(track A) vs GPT-5.6 Sol 중심(track B)" 이원 운영의 B안. 1차 출처(OpenAI 발표표 — 경쟁사 열 포함)에서 축이 깨끗하게 갈렸다: **Sol = 장기 워크플로 완주(Agents' Last Exam 52.7 vs Fable 40.5)·터미널 SOTA(88.8)·컴퓨터 사용·사이버(CTF 96.7)** / **Fable = 레포급 코딩(SWE-Pro 80.0 vs 64.6, 자사표 인정)·지능지수(AA-Int 59.9)·툴 사용(Toolathlon 61.7 vs 58)·최고난도 수학(FrontierMath T4 87.8)**. 그래서 Sol이 본체·실행을 잡고 Fable이 계획을, Opus가 1M 장문맥 레인(architect)을, grok이 3족 비평을 맡는다. **default=Anthropic 원칙의 첫 명시 예외**(validate-profiles `NON_ANTHROPIC_DEFAULT_OK` allowlist — WARN으로 트레이드오프 영구 표면화): 라우터 ctx가 272K(1M 아님)이고 툴콜 축이 열세라, **장문맥·툴 신뢰성 중심 세션은 track A가 여전히 기본**이다. L3 rolefit 사전(事前) 출하 — 실측 후 조정 예정. 근거: [evidence/2026-07-10-two-track-sol-notes.md](./evidence/2026-07-10-two-track-sol-notes.md).
...
설치기가 하는 일: `~/.gjc/agent/models.yml` 에 14개 프로필 안전 병합(재실행 시 자동 갱신) · 기존 파일 자동 백업 · 기본 프로필 `daily` 설정. `curl` + `python3` 만 있으면 된다.
...
| 장기 자율 워크플로·터미널·사이버 (272K로 충분) | `ultimate-sol` 🌞 (투트랙 B·실험적) |
| 대량 리팩터·마이그레이션 | `eco` |
...
| ultimate-sol | ●●●●○ | default·executor Sol(구독 쿼터) + planner Fable(7/12 후 credits — 저빈도 좌석) + architect Opus |
