...
- 단일 메시지(`@file`) 입력 한도는 **모델 세대에 따라 다르다**(메시지 크기 한도이지 윈도우가 아님).
  Opus 4.8 기준 실측은 350k ✅ / 476k 🔴 였다([딥다이브 §6-3](./docs/deep-dive-role-fit.md#6-3-잔여-공백-검토-gap-13--gjc-실효-컨텍스트-실측)).
  **Opus 5 는 476k 도 통과한다**(2026-08-17 배터리, needle 회수 성공 — `evidence/2026-08-17-selectors.md`). `xai/grok-4-fast:high` 도 476k 통과.
- **거대 입력은 한 메시지에 통째로 붓지 말고 청크로 나눠 누적**시키면 1M 윈도우 안에서 정상 처리된다.
  1M nominal window ≠ 완전 recall — monorepo 번들도 청크 누적 워크플로를 전제한다.
  굳이 한 방에 >400k 를 paste 해야 하면 **`anthropic/claude-opus-5:high`**(476k 실측 통과) 또는 `xai/grok-4-fast:high`(476k 실측 통과)를 쓴다.
  `deepseek-v4-pro` 도 과거 476k 를 통과했으나 2026-08-16·17 이 계정에서 403 China opt-in 이라 권장하지 않는다. `glm-5.2` 는 카탈로그 1M/131K 이지만 **대용량 단일 메시지 실측이 없다** — 카탈로그 윈도우는 단일 메시지 한도를 증명하지 않는다.
