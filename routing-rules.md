...
- **거대 입력은 한 메시지에 통째로 붓지 말고 청크로 나눠 누적**시키면 1M 윈도우 안에서 정상 처리된다.
  ...
  굳이 한 방에 >400k를 paste해야 하는 경우 **현재 권장 셀렉터가 없다.** 청크 누적으로 우회하라.
  `deepseek-v4-pro` 만 476k 단일 메시지 실수용을 확인했으나 2026-08-16·17 이 계정에서 403 China opt-in 이다. `glm-5.2` 는 카탈로그 1M/131K 이지만 **대용량 단일 메시지 실측이 없다** — 카탈로그 윈도우는 단일 메시지 한도를 증명하지 않는다(재평가 트리거: 대용량 프로브 기록).
