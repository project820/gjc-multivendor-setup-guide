...
**実証済み —— 本リポジトリが自己検証した**
> PR #4〜#7 でレビューゲートがマージ前に**実際の欠陥 11 件を遮断**した。レビュー補助スクリプトは*自身の* prompt-injection 欠陥で BLOCK され(修正後に通過)、本体(Anthropic)が自家系統の文書を甘く通した 2 点(相対パス注入面·権限の過大表示)を**cross-family critic(GPT-5.5)が正確に BLOCK** —— 自己選好バイアス防御が実戦で機能することを証明した。
