...
**実証済み — 本リポジトリが自己検証した**
> PR #4〜#7 でレビューゲートがマージ前に**欠陥 10 件を遮断**した（#4：5 · #6：5 · #7：初回投票で通過）。レビュー補助スクリプトは*自身の* prompt-injection 欠陥で BLOCK され（修正後に通過）、本体（Anthropic）が自家系統の文書を甘く通した 2 点（相対パス注入面・権限の過大表示）を **cross-family critic（当時の席 GPT-5.5 — 現行 critic 席は GPT-5.6 Sol）が正確に BLOCK**。（さらに 1 件が #6 マージ**後**に検出 → #7 で即修正。）自己選好バイアス防御が実戦で機能することを証明した。
...
### Extragoal — GPT-5.5 Pro 最終レビューレーン（opt-in）
...
| 役割 | 何をするか | 最適モデル |
|---|---|---|
| 🧠 **推論・設計**（planner） | 手順・受け入れ基準 | **GPT-5.6 Sol**（Agents' Last Exam 52.7 · 2026-07-09 GA）† — バンドル別の席は[§5](#5-️-最終カタログ--10-バンドル--4階層)を参照（例外：cyber-cop・monorepo=Gemini、eco=Luna） |
...
> **ベンチマーク基準日：2026-07-10**（planner 行 — GPT-5.6 Sol GA を反映）。† 07-02 スナップショット：planner 軸の旧首位は Gemini 3.1 Pro（GPQA 94.3 / ARC-AGI-2 77.1）だったが、07-09 Sol GA で世代交代した（[§6-2](#6-2-役割配置の最適性レビューdeep-research--実測)）。architect 軸は 2026-07-02 基準を維持 — Gemini 3.5 Pro（2M 文脈、Deep Think）は 7月 GA に延期、リリース時に再検証予定。

> 1ベンダーで5役割を埋めると、必ずどこかが最強でなくなる。本ガイドはその5枠を各々の最適ベンダーで埋め、コスト・アクセス性・信頼性まで勘案して**実際に動く**組み合わせにまとめた。v1 は3本の独立ディープリサーチ（GPT-5.5 · Claude Opus 4.8 · Gemini 3.1 Pro）、v2 は2軸ブラインド独立リサーチ（Claude Fable 5 Ultracode · Parallel.ai Ultra 2x、2026-07-10）を相互検証し（[§6-2](#6-2-役割配置の最適性レビューdeep-research--実測)）、セレクタ検証状態を[§6](#6--検証マトリクス)に明記している。
...
| gemini | `google-antigravity` | **Google AI Pro/Ultra サブスクトークン**。Gemini + 同梱 Claude（Opus 4.6 — 2026-07-02 時点、以後の同梱構成は未再確認） |
...
>
> 代替経路：`google-vertex`（API キー、トークン従量課金、1M 文脈）— サブスク/クォータに依存しないフォールバック。もう一つは **DeepInfra**（API キー）：DeepSeek V4 Pro **$1.30/$2.60**（1M 文脈）· V4 Flash $0.09/$0.18 — Standard/Priority（1.5×）ティアが GJC の `service-tier` 設定と直結する。
...
| planner（長期ワークフロー・推論） | **GPT-5.6 Sol**† | Agents' Last Exam 52.7（5.5：46.9）· AA Intelligence 58.9 — GPQA 単独首位は Sonnet 5 の 96.2 · 科学知識特化は Gemini 3.1 Pro の 94.3（[§6-2](#6-2-役割配置の最適性レビューdeep-research--実測)） |
...
> † planner 行は**2026-07-10 基準**（GPT-5.6 Sol 07-09 GA — ARC-AGI-2 の 5.6 数値は未公開、根拠は公式評価表+AA 指数）。バンドル別 planner 席は §5 席表が真実（例外：cyber-cop・monorepo=Gemini、eco=Luna）。architect 軸は Gemini 3.5 Pro リリース時に再検証対象（2026-07-02 基準）。
...
> [!WARNING]
> **GJC 0.7.10~0.9.1 のプリセット rename/delete に注意**：エンジンのカスタムプリセット rename/delete 機能は `models.yml` の**コメントを全て除去**する — インストーラの管理ブロックのセンチネル（コメント）も一緒に消えて名前ベースの置換に格下げされ、**ユーザーが削除したプロファイルが再インストール時に復活**しうる（再現確認済み）。rename/delete を使った後に再インストールする際は結果を必ず確認すること。完全な除去はバックアップ復元（`cp … .bak-*`）が最も確実。（0.9.6 では未再検証 — 再現確認は 0.7.10~0.9.1。）
