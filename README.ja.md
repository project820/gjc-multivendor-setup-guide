...
この1行で **14個のプロファイルを `~/.gjc/agent/models.yml` に安全にマージ**し、既定プロファイルを `daily` に設定する。既存設定は自動バックアップされ、再実行してもクリーンに更新される。
...
| default（ツール呼び出し · 誠実性） | **Opus 4.8 / Fable 5** | ルータ品質 = 全体の上限（Fable は refusal・課金のキャベアット — [§5](#5-️-最終カタログ14プロファイル)）。v1.12: `ultimate-sol` が唯一の明示例外（ツートラック B — Sol ルータ、272K・ツール呼び出し劣勢のトレードオフ） |
| critic（独立性） | **クロスファミリー** | メタ審判 > 討論型集計 |

> † **後継反映の脚注**：planner 軸の後継 GPT-5.6 Sol は **GA（2026-07-09）直後に反映済み** — v1.11 で planner の世代交代、v1.12 でツートラック B（`ultimate-sol` 🌞）を新設（根拠: [evidence/2026-07-10-two-track-sol-notes.md](./evidence/2026-07-10-two-track-sol-notes.md)）。architect 軸は Gemini 3.5 Pro（2M 文脈、Deep Think）が 7月 GA に延期 — リリース時にこの表は再検証対象。
...
| 🌞 **ultimate-sol** | **ツートラック B（実験的）** — Sol がメインループ・実行 + Fable が計画 + Opus 1M レーン | 長期自律ワークフロー・ターミナル・サイバー中心、272Kで十分なとき |
...
profiles:
  ...
  daily:                               # ★ 日常の既定 (--default daily)
    ...
    model_mapping:
      ...
      executor:  openai-codex/gpt-5.6-terra:high                # コーディング特化・同価格($2.5/15)で ≈GPT-5.5級(公式 5.6 評価表)・ベンダー分散(検証✅ 07-10)
  ...
  ultimate:                            # コスト度外視、役割ごと最強 + ベンダー分散（持続可能版 — イベント版は ultimate-f5）
    ...
    model_mapping:
      ...
      planner:   openai-codex/gpt-5.6-sol:xhigh                 # 最上位推論 + OpenAI の多様性(5.5→5.6 Sol 同価格の世代交代)
      architect: google-antigravity/gemini-3.1-pro-low:high     # 1M 文脈・マルチモーダル
      critic:    xai/grok-4.5:high                              # クロスファミリーの独立批評

  ultimate-f5:                         # 🔥 イベント: Fable 5 中心 — サブスク込み ~2026-07-12 23:59 PT(7/7→7/12 延長確定、週次上限の 50% まで)、以後 usage credits($10/$50)
    required_providers: [anthropic, openai-codex, google-antigravity, xai]
    model_mapping:
      ...
      planner:   openai-codex/gpt-5.6-sol:xhigh                 # 推論軸は Fable 優位未証明(ARC-AGI-2 未公開) — GPT を維持(5.6 Sol 世代交代)
      ...
      # 7/12(23:59 PT) 以後: default を opus-4-8:high に下げれば legend と同一の持続構造

  legend:                              # 👑 Ultimate Legend — 7/12 以後も維持できる最強 (fable は default の1枠のみ)
  ...
  ultimate-sol:                        # 🌞 ツートラック B (opt-in・実験的) — GPT-5.6 Sol 中心。default が非 Anthropic となる初の明示例外 · L3 rolefit 事前出荷(OpenAI 自社表ベース — 実測後に調整)
    required_providers: [openai-codex, anthropic, xai]
    model_mapping:
      default:   openai-codex/gpt-5.6-sol:high                  # 長期ワークフロー完走1位(Agents' Last Exam 52.7 vs Fable 40.5)・トークン効率。⚠ctx 272K(1M ではない)・ツール呼び出し軸で劣勢(Toolathlon 58 vs Fable 61.7)
      executor:  openai-codex/gpt-5.6-sol:xhigh                 # ターミナル・エージェント型 SOTA(T-B 2.1 88.8・DeepSWE 72.7・CTF 96.7)。⚠リポジトリ級(SWE-Pro 64.6 vs Fable 80.0)は escalation へ
      planner:   anthropic/claude-fable-5:xhigh                 # 知能・深層推論軸1位(AA-Int 59.9・GDPval 1759.6・FrontierMath T4 87.8) — cross-family 計画
      architect: anthropic/claude-opus-4-8:high                 # 1M ctx・MRCR 76% — 272K 本体の長文脈補完レーン(サブスク込み)
      critic:    xai/grok-4.5:high                              # 3系統を維持 — executor(gpt)・planner(claude) の双方と cross-family
      # track 境界: リポジトリ級コーディング・1M セッション・ツール呼び出し信頼性 = track A(ultimate/legend/escalation)
  ...
  coding-sprint:                       # 純粋な実装スループット。executor 主役 + コード理解 critic
    ...
    model_mapping:
      ...
      critic:    openai-codex/gpt-5.6-terra:high                # コード理解 critic(実バグ検出)、クロスファミリー vs gemini
  ...
  cyber-cop:                           # 🚨 reviewer モード — PRレビュー·セキュリティ監査（author モードの逆相）
    ...
    model_mapping:
      ...
      executor:  openai-codex/gpt-5.6-sol:high                  # 脇役 — 再現 PoC·failing test·harness（Terminal-Bench 2.1 88.8 SOTA）
      ...
      critic:    openai-codex/gpt-5.6-sol:high                  # 主役2 — マージゲート、Claude 作成コードと cross-family
      # 高リスク PR·セキュリティ監査：critic 3票並列パネル {openai-codex/gpt-5.6-sol:high, xai/grok-4.5:high,
      # google-antigravity/gemini-3.1-pro-low:high} — 独立投票後に本体が集計（議論禁止）、2/3 反対または CRITICAL/BLOCK 1件で遮断
      # (3票目の grok は xai ログイン時 — 未保有でも 2票 {gpt-5.6-sol, gemini} で provenance 最低条件(non-default family ≥2)を充足)
  ...
  monorepo:                            # 巨大コードベース — 全域 1M 文脈 (★gpt-5.5/5.6 272K 除外 — gpt-5.4 は 1M だが Opus が同等以上)
  ...
  solo-openai:                         # ChatGPT(Codex)アカウントのみ — base GPT 専用
    ...
    model_mapping:
      default:   openai-codex/gpt-5.6-sol:high                  # ルータ(base GPT 最強、文脈 272K)
      executor:  openai-codex/gpt-5.6-sol:xhigh                 # このアカウント最強のコーダー
      planner:   openai-codex/gpt-5.6-sol:xhigh                 # 最上位推論
      architect: openai-codex/gpt-5.4:high                      # ★gpt-5.4 = 1M 文脈 — 大容量入力はこちら(5.5/5.6 は 272K)
      critic:    openai-codex/gpt-5.6-terra:high                # ⚠同一ベンダー=独立性が弱い(トレードオフ)。Sol(executor)とモデル分離
  ...
  claude-codex:                        # ★2ベンダー(Claude+Codex)のみ — 日常均衡。Anthropic=実行・文脈 / Codex=推論・批評
    ...
    model_mapping:
      ...
      planner:   openai-codex/gpt-5.6-sol:high                  # OpenAI 推論フラッグシップ
      architect: anthropic/claude-opus-4-8:high                 # 1M ウィンドウ(gpt-5.5/5.6 272K 制限を回避)
      critic:    openai-codex/gpt-5.6-terra:high                # クロスファミリー vs Opus(executor)、コード理解
  ...
  claude-codex-max:                    # 2ベンダー(Claude+Codex)最強 — コスト度外視
    ...
    model_mapping:
      ...
      critic:    openai-codex/gpt-5.6-sol:high                  # クロスファミリーの独立批評 vs Opus

# ─────────────────────────────────────────────────────────────────────────────
# Claude 5 ファミリー(2026-07-01~): Fable 5 = $10/$50(Opus の2倍)。サブスク込みは ~7/12 23:59 PT(7/7→7/12 延長確定、週次上限の 50%)のみ — 「無料」ではない。
...
# GPT-5.6(2026-07-09 GA): Sol $5/$30 · Terra $2.5/$15(≈GPT-5.5級を半額で) · Luna $1/$6(未採用) — API 1.05M ctx/128K out、codex 表記 272K。
#   実呼び出し検証(2026-07-10, gjc 0.9.5): 3種 :medium/:high/:xhigh/:max 受理 · :bogus 拒否。:max 深度未検証 — 出荷上限 xhigh。
#   ⚠ METR: Sol の SWE 評価ゲーミング発覚 — SWE 系ベンチ単独根拠での昇格禁止(Terminal-Bench 2.1 88.8 SOTA は公式+AA クロス確認)。
...
各プロファイルの設計理由（v1.4 新規：`ultimate-f5` のイベント条件 — **サブスク込みは ~2026-07-12 23:59 PT・週次上限の 50% までで「無料」ではない** — と refusal（HTTP 200 + `stop_reason: refusal`）への注意、`legend` の持続構造と `display_name: legend5` の保護、v1.12 新設 `ultimate-sol` 🌞 ツートラック B（opt-in・実験的）：Sol がメインループ・実行（長期ワークフロー完走・ターミナル SOTA）、Fable が計画、Opus が 1M レーン — default=Anthropic 原則の唯一の明示例外（272K ルータ・ツール呼び出し劣勢のトレードオフを WARN として常時表面化、L3 rolefit 事前出荷）、`escalation` の「Fable 5 救援投手」再設計と旧 critic `:xhigh` no-op の訂正、`solo-anthropic` 全 Opus 化 — 研究上、**強いモデルほど自己選好バイアスはむしろ強く**（arXiv [2410.21819](https://arxiv.org/abs/2410.21819) · [2604.22891](https://arxiv.org/abs/2604.22891)）、能力はバイアスを相殺しない。全 Opus は「能力優先」の選択であり、**品質経路はあくまでクロスファミリー構成**）、ニーズ別チートシート、完全なディープリサーチ分析は、**[韓国語の正本 README](./README.md#5-️-최종-카탈로그-14종)** と公式 **[GJC ドキュメント](https://github.com/Yeachan-Heo/gajae-code/blob/dev/docs/multi-vendor-profiles.md)** を参照。
...
インストーラの動作：14プロファイルを `~/.gjc/agent/models.yml` に安全マージ（再実行で自動更新）・既存ファイルを自動バックアップ・既定プロファイルを `daily` に設定。`curl` + `python3` だけで動く。
...
| 長期自律ワークフロー・ターミナル・サイバー（272Kで十分） | `ultimate-sol` 🌞（ツートラック B・実験的） |
...
| ultimate-sol | ●●●●○ | default・executor Sol（サブスククォータ）+ planner Fable（7/12 後は credits — 低頻度席）+ architect Opus |
...
**v1.12.0** · [CHANGELOG](./CHANGELOG.md) · [保守プレイブック](./MAINTAINING.md) · ライセンス [CC BY 4.0](./LICENSE) · GJC = [Gajae Code](https://github.com/Yeachan-Heo/gajae-code)
