...
[![Version](https://img.shields.io/badge/version-1.12.0-2496ED?style=flat-square)](./CHANGELOG.md)
...
![Profiles](https://img.shields.io/badge/profiles-14-blue?style=flat-square)
...
- The critic runs **cross-family vs the code author (assumed Claude)** (GPT-5.6 Sol — v1.11 same-price generational replacement for gpt-5.5) → structurally counters self-preference bias ([arXiv 2410.21819](https://arxiv.org/abs/2410.21819)).
...
This single line **safely merges 14 profiles into `~/.gjc/agent/models.yml`** and sets `daily` as the default profile. Your existing config is backed up automatically, and re-running cleanly updates in place.
...
GPT 5.4 / 5.5 / 5.6 (base) low medium high xhigh             ← 5.5/5.6 default to xhigh; 5.6 :max accepted but not shipped
...
| gpt | `openai-codex` | **ChatGPT account → serves base GPT (gpt-5.6 sol/terra/luna · 5.5 · 5.4)**. ctx: gpt-5.4=1M · 5.5/the three 5.6 models=272K |
...
| default (tool-calling · honesty) | **Opus 4.8 / Fable 5** | router quality = whole-system ceiling (Fable carries refusal/billing caveats — [§5](#5-️-final-catalog-14-profiles)). v1.12: `ultimate-sol` is the only explicit exception (two-track B — Sol router, 272K/tool-call tradeoff) |
| critic (independence) | **cross-family** | meta-judge > debate aggregation |

> † **Successor reflected**: the planner-axis successor GPT-5.6 Sol was reflected immediately after GA (2026-07-09) — v1.11 swapped planner seats to the new generation, and v1.12 adds two-track B (`ultimate-sol` 🌞) (basis: [evidence/2026-07-10-two-track-sol-notes.md](./evidence/2026-07-10-two-track-sol-notes.md)). On the architect axis, Gemini 3.5 Pro (2M ctx, Deep Think) slipped to July GA; this table will be re-verified on release.
...
## 5. 🗂️ Final catalog (14 profiles)

<div align="center">
<img src="assets/profiles-matrix.svg" alt="profiles × roles matrix" width="100%">
</div>

> ★ = everyday recommendation. The top banner = the **`legend` setup** (sustainable strongest — only `default` on Fable 5). The everyday cost-balanced pick is **`daily`** (executor·critic swapped to cheaper). Multi-vendor profiles keep `default = Anthropic flagship (Opus/Fable)` and `critic = cross-family` (solo-* use the single vendor's strongest); all pass the engine effort hard-rules and **track selector verification status** ([§6](#6--verification-matrix); the 2026-07-10 rerun re-verified **all providers** — gpt-5.6 trio newly verified, Anthropic rate-limit lifted).
...
| 🔥 **ultimate-f5** | Fable 5-centric event edition — **subscription-included through 2026-07-12 23:59 PT (extension confirmed)** | peak accuracy during the event window |
| 👑 **legend** | only the main loop on Fable 5; the rest stays sustainable | strongest setup that remains viable after 7/12 |
| 🌞 **ultimate-sol** | **two-track B (experimental)** — Sol main loop & execution + Fable planning + Opus 1M lane | long autonomous workflows · terminal · cyber focus, when 272K is enough |
...
| 🗺️ **monorepo** | ≥1M ctx everywhere (gpt-5.5/5.6 excluded) | huge codebases |
| 🧱 **solo-anthropic** | every role on Opus (v1.4: critic on Opus too) | single-vendor operation |
| 🤖 **solo-openai** | every role on base GPT (5.6 Sol primary · 5.4=1M lane) | ChatGPT-only subscriber |
...
profiles:
  ...
  daily:                               # ★ everyday default (--default daily)
    ...
    model_mapping:
      ...
      executor:  openai-codex/gpt-5.6-terra:high                # coding-focused · same price ($2.5/15) for ≈GPT-5.5 class (official 5.6 eval table) · vendor spread (verified✅ 07-10)
  ...
  ultimate:                            # cost-no-object, best per role + vendor spread (sustainable edition — event edition is ultimate-f5)
    ...
    model_mapping:
      ...
      planner:   openai-codex/gpt-5.6-sol:xhigh                 # top reasoning + OpenAI diversity (5.5→5.6 Sol same-price generation swap)
  ...
  ultimate-f5:                         # 🔥 event: Fable 5-centric — subscription-included through ~2026-07-12 23:59 PT (7/7→7/12 extension confirmed, 50% weekly-limit cap), then usage credits ($10/$50)
    required_providers: [anthropic, openai-codex, google-antigravity, xai]
    model_mapping:
      ...
      planner:   openai-codex/gpt-5.6-sol:xhigh                 # reasoning axis unproven for Fable (ARC-AGI-2 unpublished) — keep GPT (5.6 Sol generation swap)
      ...
      # after 7/12 (23:59 PT): lower default to opus-4-8:high for the same sustainable structure as legend

  legend:                              # 👑 Ultimate Legend — strongest that remains viable after 7/12 (fable on the default seat only)
  ...
  ultimate-sol:                        # 🌞 Two-track B (opt-in·experimental) — centered on GPT-5.6 Sol. first explicit non-Anthropic default exception · L3 rolefit pre-shipped (based on OpenAI's own table — adjust after measurement)
    required_providers: [openai-codex, anthropic, xai]
    model_mapping:
      default:   openai-codex/gpt-5.6-sol:high                  # #1 long-workflow completion (Agents' Last Exam 52.7 vs Fable 40.5) · token efficiency. ⚠ctx 272K (not 1M) · weaker tool-call axis (Toolathlon 58 vs Fable 61.7)
      executor:  openai-codex/gpt-5.6-sol:xhigh                 # terminal/agentic SOTA (T-B 2.1 88.8 · DeepSWE 72.7 · CTF 96.7). ⚠repo-grade coding (SWE-Pro 64.6 vs Fable 80.0) goes to escalation
      planner:   anthropic/claude-fable-5:xhigh                 # #1 intelligence/deep-reasoning axis (AA-Int 59.9 · GDPval 1759.6 · FrontierMath T4 87.8) — cross-family planning
      architect: anthropic/claude-opus-4-8:high                 # 1M ctx · MRCR 76% — long-context supplement lane for the 272K main loop (subscription-included)
      critic:    xai/grok-4.5:high                              # preserve 3 families — cross-family vs both executor (gpt) and planner (claude)
      # track boundary: repo-grade coding · 1M sessions · tool-call reliability = track A (ultimate/legend/escalation)
  ...
  coding-sprint:                       # implementation throughput. executor-led + coding-aware critic
    ...
    model_mapping:
      ...
      critic:    openai-codex/gpt-5.6-terra:high                # coding-aware critic (catches real bugs), cross-family vs gemini
  ...
  cyber-cop:                           # 🚨 reviewer mode — PR review & security audit (inverse of author mode)
    ...
    model_mapping:
      ...
      executor:  openai-codex/gpt-5.6-sol:high                  # supporting role — repro PoCs, failing tests, harnesses (Terminal-Bench 2.1 88.8 SOTA)
      ...
      critic:    openai-codex/gpt-5.6-sol:high                  # lead #2 — merge gate, cross-family vs Claude-authored code
      # High-risk PRs / security audits: 3-vote parallel critic panel {openai-codex/gpt-5.6-sol:high, xai/grok-4.5:high,
      ...
      # (3rd vote (grok) requires xai login — without it, 2 votes {gpt-5.6-sol, gemini} still meet the provenance minimum of ≥2 non-default families)
  ...
  monorepo:                            # huge codebases — ≥1M ctx everywhere (★gpt-5.5/5.6 272K excluded — gpt-5.4 is 1M but Opus ranks at least equal)
  ...
  solo-openai:                         # ChatGPT (Codex) account only — base GPT only
    ...
    model_mapping:
      default:   openai-codex/gpt-5.6-sol:high                  # router (strongest base GPT, ctx 272K)
      executor:  openai-codex/gpt-5.6-sol:xhigh                 # this account's strongest coder
      planner:   openai-codex/gpt-5.6-sol:xhigh                 # top reasoning
      architect: openai-codex/gpt-5.4:high                      # ★gpt-5.4 = 1M ctx — route large inputs here (5.5/5.6 are 272K)
      critic:    openai-codex/gpt-5.6-terra:high                # ⚠same vendor = weak independence (tradeoff). Split from Sol (executor)
  ...
  claude-codex:                        # ★Claude+Codex (2 subs) only — everyday balance. Anthropic = execution/ctx, Codex = reasoning/critique
    ...
    model_mapping:
      ...
      planner:   openai-codex/gpt-5.6-sol:high                  # OpenAI reasoning flagship
      architect: anthropic/claude-opus-4-8:high                 # 1M window (avoids gpt-5.5/5.6 272K limit)
      critic:    openai-codex/gpt-5.6-terra:high                # cross-family vs Opus (executor), coding-aware
  ...
  claude-codex-max:                    # Claude+Codex (2 subs) strongest — cost-no-object
    ...
    model_mapping:
      ...
      critic:    openai-codex/gpt-5.6-sol:high                  # cross-family independent critique vs Opus

# ─────────────────────────────────────────────────────────────────────────────
# Claude 5 family (2026-07-01–): Fable 5 = $10/$50 (2× Opus); subscription-included only through ~7/12 23:59 PT (7/7→7/12 extension confirmed, 50% weekly limit) — NOT "free".
...
# GPT-5.6 (2026-07-09 GA): Sol $5/$30 · Terra $2.5/$15 (≈GPT-5.5 class at half price) · Luna $1/$6 (not adopted) — API 1.05M ctx/128K out, codex shows 272K.
#   Live-call verification (2026-07-10, gjc 0.9.5): all 3 accept :medium/:high/:xhigh/:max · reject :bogus. :max depth unverified — ship cap xhigh.
#   ⚠ METR: Sol caught gaming SWE evaluations — do not promote on SWE-style benchmarks alone (Terminal-Bench 2.1 88.8 SOTA is official+AA cross-checked).
...
For the per-profile design rationale — including the `ultimate-f5` event terms (**subscription-included through 2026-07-12 23:59 PT**, extended from the planned 7/7 end date, capped at 50% of weekly limits, then usage credits $10/$50 — never "free"), the `legend` default-seat-only Fable placement, the v1.12 `ultimate-sol` 🌞 two-track B (opt-in·experimental): Sol owns the main loop and execution (long-workflow completion + terminal SOTA), Fable plans, Opus provides the 1M lane — the only explicit exception to the default=Anthropic principle (WARNs always surface the 272K router/tool-call tradeoff, L3 rolefit pre-shipped), the `escalation` rescue-pitcher redesign (and the honest record that its v1.3 critic `:xhigh` was a no-op clamp), and the all-Opus `solo-anthropic` caveat (research shows **stronger models exhibit stronger self-preference bias** — arXiv [2410.21819](https://arxiv.org/abs/2410.21819) · [2604.22891](https://arxiv.org/abs/2604.22891) — so all-Opus is a capability-first choice and **cross-family profiles remain the quality path**) — plus the opencode-go extras, see the **[Korean canonical README](./README.md#5-️-최종-카탈로그-14종)** and the official **[GJC docs](https://github.com/Yeachan-Heo/gajae-code/blob/dev/docs/multi-vendor-profiles.md)**.

---

## 6. ✅ Verification matrix

> On 2026-07-10 (gjc 0.9.5) the core selectors of **every provider** were **actually called** via `gjc -p --no-session --no-tools --model <sel> "..."` (`evidence/2026-07-10-selectors-rerun-2.md`) — the Anthropic rate-limit lifted so every seat is green again, and the gpt-5.6 trio was verified for the first time. "Works" means a real call, not a guess.
...
| `openai-codex` | `gpt-5.6-sol:medium`/`:high`/`:xhigh`(07-10✅) · `gpt-5.6-terra:high`/`:xhigh`(07-10✅) · `gpt-5.6-luna:high`(07-10✅) · all 3 accept `:max` but depth is unverified (not shipped) · `gpt-5.5:high`(07-02✅, retired from profiles in v1.11 — kept as canary) · `gpt-5.4:high`(1M ctx lane) |
...
| `google-antigravity` | `gemini-3.1-pro-low` (±`:high`, 07-10✅) · `gemini-3.5-flash-low` (**literal pin**, 07-10✅) · `gemini-3.5-flash` (works via fuzzy matching) · ⚠`gemini-3.1-pro-high` since 0.9.5 no longer 400s but **silently fuzzy-resolves to `-low` at default effort** (NOT high reasoning — a trap) · `gemini-3-flash` · `claude-opus-4-6-thinking` (06-18 — bundle composition not re-checked since the Claude 5 launch) |
...
> [!WARNING]
> **Selectors that did NOT work here** (avoid): `openai-codex/gpt-5.3-codex`·`gpt-5.2-codex`·`gpt-5.1-codex-max`·`gpt-5.1-codex-mini` (unsupported on ChatGPT accounts) · `google-antigravity/gemini-3.1-pro-high` (**since 0.9.5 it no longer 400s — it silently fuzzy-resolves to `-low` at default effort, so it "works" but is NOT high reasoning**; the engine uses `gemini-3.1-pro-low:high`) · `gemini-3-pro` (retired) · `claude-sonnet-4-6-thinking` (404) · `gpt-oss-120b` (500). `opencode-go/*` fails **only when `OPENCODE_API_KEY` is unset** (works per the table once set). Note: `fable-5:max`·`sonnet-5:xhigh/max`·`grok-4.5:xhigh/max` are not failures — they **silently clamp** ([§3-2](#3-2-effort-cheatsheet)) — and the gpt-5.6 trio accepts `:max` but its depth is unverified (not shipped).
...
> **Deep role-placement review and the GJC effective-context measurements** (§6-2 / §6-3 in the Korean canonical) confirmed the skeleton and drove the v1.4 changes: the executor axis flipped to **Fable 5** (SWE-bench Verified 95.0 vs Opus 4.8 88.6 — Opus stays "strongest subscription-included"); the planner reasoning axis splits (GPQA is saturated — standalone #1 is now Sonnet 5 at 96.2 — while GPT-5.5 clearly led ARC-AGI-2 fluid reasoning in the dated analysis, so planner stayed GPT). **2026-07-10 update:** GPT-5.6 Sol is GA and replaces planner seats in v1.11 at the same $5/$30 price; ARC-AGI-2 for 5.6 remains unpublished, so the basis is the official eval table + AA index. `gemini-3.1-pro-low:high` invokes Gemini's native high-reasoning mode (not a degraded one); Opus holds 1M-context retrieval where Gemini collapses (hence monorepo architect = Opus, and the "Grok 2M architect" swap was rejected outright); and the single-message `@file` input cap (~400k on anthropic/antigravity) is separate from the 1M context window — chunk huge inputs across turns to keep the full window. Full tables: **[Korean README §6](./README.md#6--검증-매트릭스)**.
...
What the installer does: safely merges the 14 profiles into `~/.gjc/agent/models.yml` (auto-updates on re-run), backs up existing files, and sets `daily` as default. Needs only `curl` + `python3`.
...
| accuracy above all (Fable 5 event through 2026-07-12) | `ultimate-f5` → afterwards `legend` |
| long autonomous workflows · terminal · cyber (272K enough) | `ultimate-sol` 🌞 (two-track B · experimental) |
...
- critic panel example: `{xai/grok-4.5:high, openai-codex/gpt-5.6-terra, google-antigravity/gemini-3.1-pro-low:high}` in parallel → discard if 2/3 reject.
...
Gemini (`google-antigravity`) runs on the **Google AI Pro/Ultra subscription token** (included in the subscription, not per-token billed). **Fable 5 is included in Claude subscriptions (Pro/Max/Team) through 2026-07-12 23:59 PT** (the planned 7/7 end was extended by 5 days) — capped at 50% of your weekly usage limits — and **billed separately via usage credits afterwards**; it is **not "free."** The rest are per-token; key model prices ($/1M, in/out):
...
> † Fable 5 is exactly 2× Opus pricing. The subscription-included window (through ~7/12) still consumes your weekly limits.
...
|---|---|---|
| ultimate-f5 | ●●●●● | default·executor Fable 5 — subscription-included through ~7/12 (50% weekly-limit cap), then credits $10/50 |
| legend | ●●●●● | default Fable 5 (credits after 7/12) + executor Opus `:max` |
| ultimate / escalation | ●●●●● | executor Opus `:max` / Fable `:xhigh` (rescue pitcher) + planner GPT-5.6 Sol `:xhigh` |
| ultimate-sol | ●●●●○ | default·executor Sol (subscription quota) + planner Fable (credits after 7/12 — low-frequency seat) + architect Opus |
...
**v1.12.0** · [CHANGELOG](./CHANGELOG.md) · [Maintenance playbook](./MAINTAINING.md) · License [CC BY 4.0](./LICENSE) · GJC = [Gajae Code](https://github.com/Yeachan-Heo/gajae-code)
