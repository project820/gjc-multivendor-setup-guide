...
以下按 **GJC 0.9.1 实际生效值**记载 —— 有些地方与 API 官方规格不同（见下方脚注）：
...
>
> **脚注（上游缺口）**：按 API 官方规格，Claude 5 家族两个模型都支持到 `max`。这是 GJC 0.9.1 解析器不认识 fable 家族而回退推断、sonnet-5 继承 4.6 夹取规则的**引擎侧缺口**，已附复现材料**上报上游**。本指南按 GJC 实际生效值记载。
...
| GPT-5.6 Sol | 5 / 30（Fast 模式为 12.5/75） | planner(ultimate·legend·escalation) · cyber-cop 主角 · solo-openai |
