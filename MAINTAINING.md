...
| `scripts/cyber-cop-review.sh` | yes (`/login` + `gh`) | Headless cyber-cop reviewer-mode PR review, **seat orchestrator**: per-seat `gjc -p --model …` calls so the critic really runs on `xai/grok-4.6:high` (cross-family vs the Claude author) — not role-played by the default model (#10). Each section names its executing model; INVARIANTS run by the script itself; `--panel` adds the 2-vote high-risk critic panel `{grok, daybreak-blue}`. Never merges. |
...
**The one allowed edit: redacting a non-measurement identifier.** Append-only protects *measurements* — it is not a reason to keep a machine-local path, session id, hostname, or user name in a public repo. Such a string may be replaced with `<redacted>` provided that (a) only the identifier itself changes and every measured value on that line stays byte-identical, and (b) a dated errata appended to the same file states what was removed and why. Anything that is a measurement, a command, or an output value is still never edited — correct it by appending a dated errata instead. Applied once so far: `evidence/2026-08-17-v3-verify-baseline.md` line 30 (session UUID).
...
- **The funnel section is machine-parsed in all four READMEs — two shape rules are load-bearing.** `scripts/check-funnel-parity.py` runs on every CI build and asserts, per language, that each bundle's `required_providers` appears in exactly one row of the minimum-provider matrix and that no row names a bundle outside the roster. To locate that section it requires (1) **exactly one unnumbered `## ` heading containing 🧭** per README — the numbered `## 2. 🧭 …` section is excluded by the leading digit, so never number the funnel heading and never drop its 🧭; and (2) the matrix must be the **first contiguous table block** after that heading, with provider ids in backticks in column 1 and bundle names in `**bold**` in column 2 — the authentication-method table must stay below it, separated by a blank line, and no comment or blank line may be inserted inside the matrix. Translating the heading text is fine; changing its shape breaks CI with a message that names the file and the count it found.
  Column 2 is read as a **bundle list**: every `**lowercase-ascii**` token in it is treated as a bundle name and must exist in the roster. Do not put decorative bold ASCII words there (`**none**`, `**all**`, `**n/a**`) — write them unbolded or in Korean/Chinese/Japanese. Fences are masked before the heading scan, so a `## 🧭 …` inside a code sample is ignored.
