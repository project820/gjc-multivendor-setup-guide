...
| `scripts/cyber-cop-review.sh` | yes (`/login` + `gh`) | Headless cyber-cop reviewer-mode PR review, **seat orchestrator**: per-seat `gjc -p --model …` calls so the critic really runs on `xai/grok-4.6:high` (cross-family vs the Claude author) — not role-played by the default model (#10). Each section names its executing model; INVARIANTS run by the script itself; `--panel` adds the 2-vote high-risk critic panel `{grok, sol}`. Never merges. |
