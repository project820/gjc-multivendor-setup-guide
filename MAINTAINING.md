...
| `scripts/gjc-cop` | yes (`/login` + `gh`) | One-command reviewer wrapper shipped by `install.sh GJC_SETUP_COP=1` to `~/.local/bin/gjc-cop` (deps in `~/.gjc/agent/cyber-cop/`). `gjc-cop <PR>` / `--panel <PR>` / `shell` / `watch` — clone-free; always injects trusted local paths (kills the #6 relative-path injection class by construction). Wraps `cyber-cop-review.sh`; never merges. |
