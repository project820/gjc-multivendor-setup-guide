...
1. **`default` = strongest router.** If `anthropic` is in `required_providers`, `default` must be an **Anthropic flagship (Opus 5 / Fable 5)**. A weak router caps whole-system quality. Documented exceptions live in `scripts/validate-profiles.py` (`NON_ANTHROPIC_DEFAULT_OK` — currently `ultimate-sol` only; surfaced as WARN, never silent).
